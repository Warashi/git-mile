...
use rocksdb::{ColumnFamilyDescriptor, DB, DEFAULT_COLUMN_FAMILY_NAME, IteratorMode, Options};
...
pub trait RepositoryCacheHook: Send + Sync {
    ...
    fn on_pack_persisted(&self, _entity_id: &EntityId, _inserted: &[OperationId]) -> Result<()> {
        Ok(())
    }
...
impl PersistentCache {
    ...
    pub fn open(config: CacheConfig) -> Result<Self> {
        ...
        let mut descriptors = Vec::with_capacity(CacheNamespace::ALL.len() + 1);
...
impl RepositoryCacheHook for EntityCacheAdapter {
    ...
    fn on_pack_persisted(&self, entity_id: &EntityId, inserted: &[OperationId]) -> Result<()> {
        self.repository
            .on_pack_persisted(self.namespace, entity_id, inserted)?;
        Ok(())
    }
